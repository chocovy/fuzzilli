            ...
            let module = b.buildWasmModule { wasmModule in
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => [.wasmi32]) { f, _, _ in
                        ...
                        let loadedValue = f.wasmAtomicLoad(
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => [.wasmi64]) { f, _, _ in
                        let valueToStore = f.consti64(0x1122_3344_5566_7788)
                        let address = f.consti32(16)
                        f.wasmAtomicStore(
                            memory: memory, address: address, value: valueToStore,
                            storeType: .i64Store, offset: 0)
                        let loadedValue = f.wasmAtomicLoad(
                            memory: memory, address: address, loadType: .i64Load, offset: 0)
                        return [loadedValue]
                    }
                }
            }
            ...
            let module = b.buildWasmModule {
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => [.wasmi32]) { f, _, _ in
                        ...
                        let valueToStore = f.consti32(0x1234_56FF)
                        let address = f.consti32(4)

                        f.wasmAtomicStore(
                            memory: memory, address: address, value: valueToStore,
                            storeType: .i32Store8, offset: 0)
                        // The load will zero-extend 0xFF to 255
                        let loadedValue = f.wasmAtomicLoad(
                            memory: memory, address: address, loadType: .i32Load8U, offset: 0)
                        return [loadedValue]
                    }
                }
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => [.wasmi64]) { f, _, _ in
                        ...
                        let valueToStore = f.consti64(0x1234_5678_90AB_CDEF)
                        let address = f.consti32(4)

                        f.wasmAtomicStore(
                            memory: memory, address: address, value: valueToStore,
                            storeType: .i64Store8, offset: 0)
                        // The load will zero-extend 0xEF to 239
                        let loadedValue = f.wasmAtomicLoad(
                            memory: memory, address: address, loadType: .i64Load8U, offset: 0)
                        return [loadedValue]
                    }
                }
            }
            ...
            let module = b.buildWasmModule {
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => []) { f, _, _ in
                        ...
                        f.wasmAtomicStore(
                            memory: memory, address: address, value: value, storeType: .i32Store,
                            offset: 0)
                        return []
                    }
                }
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    wasmModule.addWasmFunction(with: [] => []) { f, _, _ in
                        ...
                        // This should trap.
                        let value = f.consti64(0xDEAD_BEEF)
                        f.wasmAtomicStore(
                            memory: memory, address: address, value: value, storeType: .i64Store,
                            offset: 0)
                        return []
                    }
                }
            ...
            let module = b.buildWasmModule { wasmModule in
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    for (op, storeType, loadType, valueType) in testCases {
                        ...
                        wasmModule.addWasmFunction(with: [] => [returnType, returnType]) {
                            f, _, _ in
                            let valueToStore =
                                (valueType == .wasmi32)
                                ? f.consti32(Int32(initialValue)) : f.consti64(initialValue)
                            let rhs =
                                (valueType == .wasmi32)
                                ? f.consti32(Int32(operand)) : f.consti64(operand)
                            let lhs = f.consti32(8)
                            f.wasmAtomicStore(
                                memory: memory, address: lhs, value: valueToStore,
                                storeType: storeType, offset: 0)
                            let originalValue = f.wasmAtomicRMW(
                                memory: memory, lhs: lhs, rhs: rhs, op: op, offset: 0)
                            let finalValue = f.wasmAtomicLoad(
                                memory: memory, address: lhs, loadType: loadType, offset: 0)
                            return [originalValue, finalValue]
                        }

                        let expectedFinal =
                            switch op {
                            case .i32Add, .i32Add8U, .i32Add16U, .i64Add, .i64Add8U, .i64Add16U,
                                .i64Add32U:
                                initialValue + operand
                            case .i32Sub, .i32Sub8U, .i32Sub16U, .i64Sub, .i64Sub8U, .i64Sub16U,
                                .i64Sub32U:
                                initialValue - operand
                            case .i32And, .i32And8U, .i32And16U, .i64And, .i64And8U, .i64And16U,
                                .i64And32U:
                                initialValue & operand
                            case .i32Or, .i32Or8U, .i32Or16U, .i64Or, .i64Or8U, .i64Or16U,
                                .i64Or32U:
                                initialValue | operand
                            case .i32Xor, .i32Xor8U, .i32Xor16U, .i64Xor, .i64Xor8U, .i64Xor16U,
                                .i64Xor32U:
                                initialValue ^ operand
                            case .i32Xchg, .i32Xchg8U, .i32Xchg16U, .i64Xchg, .i64Xchg8U,
                                .i64Xchg16U, .i64Xchg32U:
                                operand
                            }
                ...
                for memory in [unsharedMemory, sharedMemory] {
                    for (op, storeType, loadType, valueType) in testCases {
                        ...
                        wasmModule.addWasmFunction(with: [] => [returnType, returnType]) {
                            ...
                            let replacement =
                                (valueType == .wasmi32)
                                ? f.consti32(Int32(replacement)) : f.consti64(replacement)
                            let address = f.consti32(8)
                            f.wasmAtomicStore(
                                memory: memory, address: address, value: valueToStore,
                                storeType: storeType, offset: 0)
                            let originalValue = f.wasmAtomicCmpxchg(
                                memory: memory, address: address, expected: expected,
                                replacement: replacement, op: op, offset: 0)
                            let finalValue = f.wasmAtomicLoad(
                                memory: memory, address: address, loadType: loadType, offset: 0)
                            return [originalValue, finalValue]
                        }
                        ...
                        wasmModule.addWasmFunction(with: [] => [returnType, returnType]) {
                            ...
                            let expected =
                                (valueType == .wasmi32)
                                ? f.consti32(Int32(initialValue - 1)) : f.consti64(initialValue - 1)
                            let replacement =
                                (valueType == .wasmi32)
                                ? f.consti32(Int32(replacement)) : f.consti64(replacement)
                            let address = f.consti32(8)
